...
class OptiNet(QMainWindow):
    ...
    def UI(self):
        ...
        for pagina in [
            ...
            self.pagina_trafico,
            self.pagina_dispositivos,
            self.pagina_wifi,
            self.pagina_ancho_banda,
            self.pagina_reporte,
            self.pagina_left,
        ]:
            self.stack.addWidget(pagina)

        boton_principal.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_principal)
        )
        boton_trafico.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_trafico)
        )
        boton_dispositivos.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_dispositivos)
        )
        boton_wifi.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_wifi)
        )
        boton_ancho_banda.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_ancho_banda)
        )
        boton_reporte.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_reporte)
        )
        boton_left.clicked.connect(
            lambda: self.stack.setCurrentWidget(self.pagina_left)
        )
    ...
    def actualizar_tabla(self, dispositivos):
        ...
        for dispositivo in dispositivos:
            ...
            self.tabla_dispositivos.setItem(
                row_position, 0, QTableWidgetItem(dispositivo[0])
            )
            self.tabla_dispositivos.setItem(
                row_position, 1, QTableWidgetItem(dispositivo[1])
            )
